#include <bits/stdc++.h>
using namespace std;

void dfs(int row, int col, vector<vector<int>> &vis, char **board, int delrow[], int delcol[], int n, int m) {
    vis[row][col] = 1;

    for (int i = 0; i < 4; i++) {
        int nrow = row + delrow[i];
        int ncol = col + delcol[i];

        if (nrow >= 0 && nrow < n && ncol >= 0 && ncol < m && !vis[nrow][ncol] && board[nrow][ncol] == 'O') {
            dfs(nrow, ncol, vis, board, delrow, delcol, n, m);
        }
    }
}

void replaceOWithX(char **board, int n, int m) {
    // Time Complexity: O(n * m) - Each cell is visited once.
    // Space Complexity: O(n * m) - Extra space is used for the visited array.

    int delrow[] = {-1, 0, 1, 0};
    int delcol[] = {0, 1, 0, -1};

    vector<vector<int>> vis(n, vector<int>(m, 0));

    for (int j = 0; j < m; j++) {
        if (!vis[0][j] && board[0][j] == 'O') {
            dfs(0, j, vis, board, delrow, delcol, n, m);
        }

        if (!vis[n - 1][j] && board[n - 1][j] == 'O') {
            dfs(n - 1, j, vis, board, delrow, delcol, n, m);
        }
    }

    for (int i = 0; i < n; i++) {
        if (!vis[i][0] && board[i][0] == 'O') {
            dfs(i, 0, vis, board, delrow, delcol, n, m);
        }

        if (!vis[i][m - 1] && board[i][m - 1] == 'O') {
            dfs(i, m - 1, vis, board, delrow, delcol, n, m);
        }
    }

    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            if (!vis[i][j] && board[i][j] == 'O') {
                board[i][j] = 'X';
            }
        }
    }
}

int main() {
    // Example usage:
    int n = 3;
    int m = 3;
    char **board = new char *[n];
    for (int i = 0; i < n; i++) {
        board[i] = new char[m];
        for (int j = 0; j < m; j++) {
            board[i][j] = 'O';
        }
    }

    replaceOWithX(board, n, m);

    // Printing the updated board
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            cout << board[i][j] << " ";
        }
        cout << endl;
    }

    // Clean up the dynamically allocated memory
    for (int i = 0; i < n; i++) {
        delete[] board[i];
    }
    delete[] board;

    return 0;
}
